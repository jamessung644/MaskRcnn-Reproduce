"""체크포인트 -> Precision/Recall/F1 성능지표 시각화 (COCOeval 기반).

tools/evaluate.py가 뽑는 AP/AP50/AP75는 스칼라 요약값이라 "어느 정도
confidence threshold에서 precision/recall 균형이 가장 좋은가"를 안 보여준다.
이 스크립트는 COCOeval이 이미 계산해 둔 카테고리별 precision-recall-score
배열(IoU=0.5 기준)에서:
    - 카테고리 평균 Precision-Recall curve
    - 그 위에서의 F1 = 2PR/(P+R) curve
    - F1이 최대가 되는 지점(및 근사 confidence threshold)
을 뽑아 bbox/segm 각각 그래프로 저장하고 콘솔에 요약을 출력한다.
...
from maskrcnn.evaluate import evaluate_coco, load_checkpoint, precision_recall_f1_curve

# bbox/segm 2계열 고정 색상 (파랑/주황 — 색약 대비가 큰 표준 조합)
